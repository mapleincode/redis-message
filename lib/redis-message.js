...
class RedisMessage {
    ...
    async _dealFailedMessage(messageId) {
        ...
        if (failedTimes > this.maxRetryTimes) {
            ...
            // 调用 deal 函数
            await dealfailedMessage(messageId, detail);
            // 删除失败次数
    ...
    async pullMessage() {
        ...
        // Async
        // 主要是不想堵塞消息获取。
        // 但是造成的后果是前几次消息 fetch 可能为空。造成消息延迟。
        setTimeout(function() {
            self._pull();
        }, 1000);
    ...
    async getMessages(size) {
        ...
        if (mqCount < this.minRedisMessageCount) {
            const status = await this.pullMessage(); // async
            if (status === false) {
                // 没有更新
                await sleep(500);
            }
    ...
    async checkExpireMessage() {
        ...
        for (const key of keys) {
            ...
            } else if (index < 0 && hashMap[key] && (Date.now() - hashMap[key]) > this.maxAckTimeout) {
                // 数据 ack 超时
