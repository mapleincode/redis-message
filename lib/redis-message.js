...
class RedisMessage {
    constructor(options) {
        const {
            ...
            dealTimeoutMessage = function(options) {
                // const { topic, messageType } = options;
        ...
        this.dealTimeoutMessage = dealTimeoutMessage(options);
    ...
    async checkExpireMessage() {
        ...
        for (const messageId of timeoutList) {
            ...
            realTimeoutList.push(messageId);
            await this._dealFailedMessage(messageId);
        }

        try {
            if (readTimeoutList.length) {
